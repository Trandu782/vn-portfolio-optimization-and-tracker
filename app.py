...
# --- Page Configuration ---
st.set_page_config(
    page_title="VN Portfolio & Monte Carlo Optimizer", layout="wide"
)
st.title("📈 VN Portfolio & Monte Carlo Optimizer")
st.write(
    "Track your Vietnamese stock portfolio performance against the VN-Index and"
    " discover the optimal asset allocation."
)
...
with col1:
  ticker_input = (
      st.text_input("Ticker (e.g., FPT, MBB)")
      .strip()
      .upper()
      .replace(".VN", "")
  )
with col2:
  shares_input = st.number_input(
      "Shares", min_value=1.0, value=100.0, step=10.0
  )
...
if run_analysis:
  if not st.session_state["portfolio"]:
    st.warning(
        "Please add at least one stock to your portfolio in the sidebar before"
    ...
    with st.spinner("Fetching Data and Running Simulations..."):
      ...
      def fetch_dnse_close(symbol, start, end):
        ...
        # FIXED: Call datetime operations directly on DatetimeIndex without .dt
        dates = (
            pd.to_datetime(payload["t"], unit="s", utc=True)
      ...
      try:
        stock_series = {
            ticker: fetch_dnse_close(ticker, start_date, end_date)
            for ticker in portfolio_input
        }
      ...
      try:
        ...
        benchmark_data.index = (
            benchmark_data.index.tz_localize("UTC")
            .tz_convert("Asia/Ho_Chi_Minh")
            .normalize()
            .tz_localize(None)
        )
      ...
      for ticker, shares in portfolio_input.items():
        portfolio_value["Total Value"] += stock_data[ticker].ffill() * shares

      portfolio_value = portfolio_value.dropna(subset=["Total Value"])
      portfolio_normalized = (
          portfolio_value["Total Value"] / portfolio_value["Total Value"].iloc[0]
      ) * 100
      ...
      fig_perf.add_trace(
          go.Scatter(
              ...
              hovertemplate=(
                  "%{x|%Y-%m-%d}<br>Portfolio Growth: %{y:.2f}<extra></extra>"
              ),
          )
      )
      ...
      if not benchmark_close.empty:
        fig_perf.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    "%{x|%Y-%m-%d}<br>VN-Index Growth: %{y:.2f}<extra></extra>"
                ),
            )
        )
      ...
      current_values = np.array(
          [portfolio_input[t] * latest_prices[t] for t in stock_data.columns]
      )
      ...
      curr_volatility = np.sqrt(
          np.dot(current_weights.T, np.dot(cov_matrix, current_weights))
      )
      ...
      fig_opt = make_subplots(
          ...
          subplot_titles=(
              ...
              "<b>Holdings: Current vs Optimal</b>",
          ),
      ...
      fig_opt.add_trace(
          go.Scatter(
              ...
              hovertemplate=(
                  "Volatility: %{x:.2%}<br>Return: %{y:.2%}<br>Sharpe:"
      ...
      fig_opt.update_xaxes(
          title_text="Annualized Volatility (Risk)", row=1, col=1
      )
      fig_opt.update_yaxes(title_text="Expected Annual Return", row=1, col=1)
      fig_opt.update_xaxes(title_text="Stock", row=1, col=2)
